...
$id = (int) ($_GET['id'] ?? 0);
if (!$id) {
    json_response(400, ['error' => 'Missing business id']);
}

// Only admin or the business owner can upload
if ($user['role'] !== 'admin') {
...
$uploadDir = __DIR__ . '/../../uploads/businesses/';
if (!is_dir($uploadDir)) {
...
$stmt = db()->prepare('UPDATE businesses SET image_url = ? WHERE id = ?');
$stmt->execute([$imageUrl, $id]);

error_log("[businesses/image] Uploaded image for business $id: $imageUrl");
